...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Failed to create federation! Head over to @SabrinaChat to notify us of the error.")
			return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join federation! Please head to @SabrinaChat to report this.")
                return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem persists, reach out to us @SabrinaChat.")
			return
				 ...
				 "<b>FedBan reason updated</b>" \
							 ...
							 "\n<b>Reason:</b> {}".format(fed_name, mention_html(user.id, user.first_name),
									   mention_html(user_chat.id, user_chat.first_name),
													user_chat.id, reason), 
				html=True)
		message.reply_text("Done Dana Dan Done🔥")
		return
	...
	send_to_list(bot, FEDADMIN,
			 ...
			 "\n<b>Reason:</b> {}".format(fed_name, mention_html(user.id, user.first_name),
								   mention_html(user_chat.id, user_chat.first_name),
												user_chat.id, reason), 
			html=True)
	message.reply_text("<b>New FedBan</b>" \
			 "\n<b>Federation:</b> {}" \
			 "\n<b>Federation Admin:</b> {}" \
			 "\n<b>User:</b> {}" \
			 "\n<b>User ID:</b> <code>{}</code>" \
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Failed to set federation rules. If this persists, reach out to us @SabrinaChat.")
			return
